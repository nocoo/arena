...
export function HexlyLink() {
  return (
    <HeaderTooltip label="Arena on hexly.ai">
      <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-foreground" asChild>
        <a
          href="https://hexly.ai/projects/arena"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Arena on hexly.ai (opens in a new tab)"
        >
          <svg
            className="size-[18px]"
            width="18"
          ...
          <span className="sr-only">Arena on hexly.ai (opens in a new tab)</span>
        </a>
      </Button>
    </HeaderTooltip>
  );
